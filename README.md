# 目录

> 预览地址：https://bluexiaowei.github.io

## FAQ

- [apache2 proxyPass 代理配置](/docs/apache2_proxyPass.md)
- [https 证书申请并且配置 nginx ](/docs/nginx_https.md)
- [docker-compose 启动 nginx](/docs/nginx_docker.md)
- [Reactjs + AntdForm + TextArea 替换回车键](/docs/onEnterReplace.md)
- [npm 更换源](/docs/javascript_npm_source.md)

## System

- [windows 10](/docs/windows_10.md)
- [替换源](/docs/replace_source.md)
- [常用软件](/docs/software.md)

## Software

- [verdaccio：轻量级开源私有 npm 代理注册表](/docs/verdaccio.md)
- [docker：应用容器引擎](/docs/docker.md)
- [nginx：轻量级 WEB 服务器](/docs/nginx.md)
- [frp：内网穿透](/docs/frp.md)
- [swcr：计算机软件著作权程序鉴别材料（即源代码）生成器](/docs/swcr.md)
- [certbot：let’s encrypt https 证书生成器](/docs/certbot.md)
